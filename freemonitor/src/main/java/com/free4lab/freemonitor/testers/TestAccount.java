 ...
 * @Description: 测试account2.0服务
 * @author lmqt
 * @date 2013-4-9 9:35:18
 */
public class TestAccount extends TestBench {
	...
	private static final String ACCOUNT_HOST = "accounttest.free4lab.com";
	...
	public void testLogin(){
		TestAccount tester = new TestAccount();
		try {
			logger.info("test first login");
			if(tester.loginFirstTime()){
				outputclient.recordData(TYPE, "login", Const.RIGHT, "");
			}else{
				outputclient.recordData(TYPE, "login", Const.ERROR, "service unavailable");
			}
		} catch (HttpException e) {
			outputclient.recordData(TYPE, "login", Const.ERROR, e.getMessage());
		} catch (IOException e) {
			outputclient.recordData(TYPE, "login", Const.ERROR, e.getMessage());
		}
		try {
			logger.info("test second login");
			if(tester.loginSecondTime()){
				outputclient.recordData(TYPE, "sso", Const.RIGHT, "");
			}else{
				outputclient.recordData(TYPE, "sso", Const.ERROR, "service unavailable");
			}
		} catch (URIException e) {
			outputclient.recordData(TYPE, "sso", Const.ERROR, e.getMessage());
		}
		try {
			logger.info("test get access_token");
			if(tester.getAccessToken()){
				outputclient.recordData(TYPE, "access_token", Const.RIGHT, "");
			}else{
				outputclient.recordData(TYPE, "access_token", Const.ERROR, "service unavailable");
			}
		} catch (HttpException e) {
			outputclient.recordData(TYPE, "access_token", Const.ERROR, e.getMessage());
		} catch (IOException e) {
			outputclient.recordData(TYPE, "access_token", Const.ERROR, e.getMessage());
		}
		try {
			logger.info("test get userinfo");
			if(tester.getUserInfo()){
				outputclient.recordData(TYPE, "userinfo", Const.RIGHT, "");
			}else{
				outputclient.recordData(TYPE, "userinfo", Const.ERROR, "service unavailable");
			}
		} catch (HttpException e) {
			outputclient.recordData(TYPE, "userinfo", Const.ERROR, e.getMessage());
		} catch (IOException e) {
			outputclient.recordData(TYPE, "userinfo", Const.ERROR, e.getMessage());
		}
		try {
			...
			if(tester.logout()){
				outputclient.recordData(TYPE, "logout", Const.RIGHT, "");
			}else{
				outputclient.recordData(TYPE, "logout", Const.ERROR, "service unavailable");
			}
		} catch (HttpException e) {
			outputclient.recordData(TYPE, "logout", Const.ERROR, e.getMessage());
		} catch (IOException e) {
			outputclient.recordData(TYPE, "logout", Const.ERROR, e.getMessage());
		}
	}
